...
#define YPOS_STATDISPLAY  (MENUITEM_STATDISPLAY * 16)
...
static u8   StatDisplay_ProcessInput(u8 selection);
static void StatDisplay_DrawChoices(u8 selection);
...
static const u8 *const sOptionMenuItemsNames[MENUITEM_COUNT] =
{
    ...
    [MENUITEM_STATDISPLAY]   = gText_StatDisplay,
...
void CB2_InitOptionMenu(void)
{
    ...
    case 10:
    {
        ...
        gTasks[taskId].data[TD_STATDISPLAY] = gSaveBlock2Ptr->optionsStatDisplay;
        ...
        StatDisplay_DrawChoices(gTasks[taskId].data[TD_STATDISPLAY]);
    ...
    else
    {
        ...
        case MENUITEM_FRAMETYPE:
            ...
            if (previousOption != gTasks[taskId].data[TD_FRAMETYPE])
                FrameType_DrawChoices(gTasks[taskId].data[TD_FRAMETYPE]);
            break;
        case MENUITEM_STATDISPLAY:
            previousOption = gTasks[taskId].data[TD_STATDISPLAY];
            gTasks[taskId].data[TD_STATDISPLAY] = StatDisplay_ProcessInput(gTasks[taskId].data[TD_STATDISPLAY]);

            if (previousOption != gTasks[taskId].data[TD_STATDISPLAY])
                StatDisplay_DrawChoices(gTasks[taskId].data[TD_STATDISPLAY]);
            break;
        default:
            return;
...
static void Task_OptionMenuSave(u8 taskId)
{
    ...
    gSaveBlock2Ptr->optionsStatDisplay = gTasks[taskId].data[TD_STATDISPLAY];
...
static void StatDisplay_DrawChoices(u8 selection)
{
    ...
    DrawOptionMenuChoice(gText_StatDisplayLetter, 104, YPOS_STATDISPLAY, styles[0]);
    DrawOptionMenuChoice(gText_StatDisplayNumber, GetStringRightAlignXOffset(1, gText_StatDisplayNumber, 198), YPOS_STATDISPLAY, styles[1]);
}
